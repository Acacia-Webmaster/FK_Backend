 ...
 * Admin sets a new password for an admin by id (no current password required).
...
router.post('/reset-password', auth, async (req, res) => {
  try {
    ...
    if (!adminId || !newPassword) {
      return res
        .status(400)
        .json({ message: 'adminId and newPassword are required' });
    }

    // You can also restrict this so only the same admin can reset itself:
